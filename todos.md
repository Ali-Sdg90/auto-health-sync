- [x] Add setting for time of the auto backup, defaulting to 11:00 PM.

- [x] Add setting to allow user to choose between Jalali and Gregorian date in the backup file name.

- [ ] Add Setting for choose which Health Connect data types to backup, defaulting to all available types.

- [x] Add CI/CD pipeline to build and test the app on every commit.

- [x] Add CI/CD pipeline to build, test, and publish signed APK/AAB artifacts.

- [ ] Update app google cloud registration for the global release.

- [ ] Release the app on Bazaar and Myket and other places.

- [x] Add an “Open backup folder in Google Drive” action.

- [x] Add an “Open Health Connect” action.

- [ ] Add a first-run onboarding checklist for Health Connect, Drive, notifications, and background access.

- [ ] Add a system health/status check to warn when Health Connect permissions, Drive authorization, notifications, or background execution are unavailable.

- [ ] Add an About section/footer with app version, repository link, privacy policy, and Created by A.S..

- [ ] Add a privacy policy to the repository and app.

- [ ] Add Custom License to the repository and app.
